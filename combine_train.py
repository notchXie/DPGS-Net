...
import MultiTaskDataset as datasets
...
def train(root_dir):
    ...
    model = MultiTaskUnet.Unet(in_channels=3, out_channels=1)
    ...
    train_dataset = datasets.MultiTaskDataset(root_dir=root_dir, fold_ids=train_folds, csv_path=r'other/image_labels2.csv')
    train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=batch_size, shuffle=True, num_workers=12, pin_memory=True)

    test_dataset = datasets.MultiTaskDataset(root_dir=root_dir, fold_ids=test_folds, csv_path=r'other/image_labels2.csv')
    test_loader = torch.utils.data.DataLoader(test_dataset, batch_size=batch_size, shuffle=True, num_workers=12, pin_memory=True)

    criterion = MultiTaskLoss.MultiTaskLoss(alpha=0, segmentation_loss_fn=nn.BCEWithLogitsLoss(), dice_weight=0)
    ...
    for epoch in range(epoch_num):
        for i, (img, mask, class_label) in enumerate(tqdm(train_loader)):
            img = img.to(device)
            mask = mask.to(device)
            class_label = class_label.to(device)

            class_output, mask_output = model(img)

            loss = criterion(class_output, mask_output, class_label, mask)
        ...
        if epoch != 666 :
            ...
            with torch.no_grad():
                ...
                for i, (img, mask, class_label) in enumerate(test_loader):
                    img = img.to(device)
                    mask = mask.to(device)
                    class_output, output = model(img)
                    ...
                    for j in range(img.size(0)):  # 对每张图像进行评估
                        ...
                        # 计算分类准确度
                        _, predicted = torch.max(class_output[j], 0)
                        class_correct += (predicted == class_label[j]).sum().item()
            ...
            if epoch % 3 == 0:
                torch.save(model.state_dict(), f'unet_{epoch}.pth')
